...
docker build -t sdncheckerv2 .
if [[ $? -ne 0 ]]; then
    echo "Failed to build image. Exit"
    exit 1
fi
docker tag sdncheckerv2 $TEST_DOCKER_REGISTRY/$TEST_NAMESPACE/sdncheckerv2:v1
docker push $TEST_DOCKER_REGISTRY/$TEST_NAMESPACE/sdncheckerv2:v1
...
sed "s/TEST_DOCKER_REGISTRY/$TEST_DOCKER_REGISTRY/g" sdncheckerv2.yml > sdncheckerv2-test.yml
sed -i "s/TEST_NAMESPACE/$TEST_NAMESPACE/g" sdncheckerv2-test.yml
sed -i "s/TEST_ROUTE/$TEST_ROUTE/g" sdncheckerv2-test.yml
oc create -f sdncheckerv2-test.yml
